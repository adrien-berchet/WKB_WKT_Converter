...
use wkb_wkt_converter::{
    hex_wkb_to_wkt, wkb_to_wkt as core_wkb_to_wkt, wkb_to_wkt_split_srid,
...
#[test]
fn wkb_to_wkt_auto_preserves_srid() {
    let wkb = wkb_of("SRID=4326;POINT (1 2)");
    assert_eq!(
        core_wkb_to_wkt(&wkb, SridMode::Auto).unwrap(),
        "SRID=4326;POINT (1 2)"
    );
}

#[test]
fn wkb_to_wkt_strip_removes_srid() {
    let wkb = wkb_of("SRID=4326;POINT (1 2)");
    assert_eq!(
        core_wkb_to_wkt(&wkb, SridMode::Strip).unwrap(),
        "POINT (1 2)"
    );
}

#[test]
fn wkb_to_wkt_strip_without_srid_is_noop() {
...
#[test]
fn wkb_to_wkt_set_adds_srid() {
    let wkb = wkb_of("POINT (1 2)");
    assert_eq!(
        core_wkb_to_wkt(&wkb, SridMode::Set(4326)).unwrap(),
        "SRID=4326;POINT (1 2)"
    );
}

#[test]
fn wkb_to_wkt_set_overrides_srid() {
    let wkb = wkb_of("SRID=4326;POINT (1 2)");
    assert_eq!(
        core_wkb_to_wkt(&wkb, SridMode::Set(3857)).unwrap(),
        "SRID=3857;POINT (1 2)"
    );
}
...
#[test]
fn hex_wkb_to_wkt_auto_preserves_srid() {
    let hex = wkt_to_hex_wkb("SRID=4326;POINT (1 2)").unwrap();
    assert_eq!(
        hex_wkb_to_wkt(&hex, SridMode::Auto).unwrap(),
        "SRID=4326;POINT (1 2)"
    );
}

#[test]
fn hex_wkb_to_wkt_strip_removes_srid() {
    let hex = wkt_to_hex_wkb("SRID=4326;POINT (1 2)").unwrap();
    assert_eq!(
        hex_wkb_to_wkt(&hex, SridMode::Strip).unwrap(),
        "POINT (1 2)"
    );
}

#[test]
fn hex_wkb_to_wkt_strip_without_srid_is_noop() {
    let hex = wkt_to_hex_wkb("POINT (1 2)").unwrap();
    assert_eq!(
        hex_wkb_to_wkt(&hex, SridMode::Strip).unwrap(),
        "POINT (1 2)"
...
#[test]
fn hex_wkb_to_wkt_set_adds_srid() {
    let hex = wkt_to_hex_wkb("POINT (1 2)").unwrap();
    assert_eq!(
        hex_wkb_to_wkt(&hex, SridMode::Set(4326)).unwrap(),
        "SRID=4326;POINT (1 2)"
    );
}

#[test]
fn hex_wkb_to_wkt_set_overrides_srid() {
    let hex = wkt_to_hex_wkb("SRID=4326;POINT (1 2)").unwrap();
    assert_eq!(
        hex_wkb_to_wkt(&hex, SridMode::Set(3857)).unwrap(),
        "SRID=3857;POINT (1 2)"
    );
}
...
fn hex_wkb_odd_length_error() {
    for mode in [SridMode::Auto, SridMode::Strip, SridMode::Set(4326)] {
        assert!(hex_wkb_to_wkt("ABC", mode).is_err());
    }
}
...
fn hex_wkb_invalid_chars_error() {
    for mode in [SridMode::Auto, SridMode::Strip, SridMode::Set(4326)] {
        assert!(hex_wkb_to_wkt("ZZZZ", mode).is_err());
    }
}

#[test]
fn hex_wkb_truncated_errors_in_all_srid_modes() {
    for mode in [SridMode::Auto, SridMode::Strip, SridMode::Set(4326)] {
        assert!(hex_wkb_to_wkt("0101000000", mode).is_err());
    }
}
...
fn truncated_wkb_errors() {
    ...
    for mode in [SridMode::Auto, SridMode::Strip, SridMode::Set(4326)] {
        assert!(core_wkb_to_wkt(&wkb, mode).is_err());
    }
}
...
fn top_level_trailing_bytes_error() {
    ...
    let hex = hex::encode_upper(&wkb);
    for mode in [SridMode::Auto, SridMode::Strip, SridMode::Set(4326)] {
        assert!(hex_wkb_to_wkt(&hex, mode).is_err());
    }
}
